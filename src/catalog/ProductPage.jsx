    ...
    return (
      <PublicLayout>
        <div className={styles.notFound}>
          ...
          <Link to="/catalogo" className={styles.backLink}>← Volver al catálogo</Link>
        </div>
      </PublicLayout>
    )
  }
  ...
  return (
    <PublicLayout>
      <div className={`container ${styles.page}`}>
        <Link to="/catalogo" className={styles.breadcrumb}>← Catálogo</Link>
        ...
        <div className={styles.grid}>
          ...
          <div className={styles.infoCol}>
            {product.categoryName && (
              <span className="label-caps">{product.categoryName}</span>
            )}
            <h1 className={styles.name}>{product.name}</h1>
            {product.price && (
              <p className={styles.price}>
                {product.currency ?? '$'}{Number(product.price).toLocaleString('es-AR')}
              </p>
            )}
            ...
            {product.attributes?.length > 0 && (
              <div className={styles.attributes}>
                {product.attributes.map((attr, i) => (
                  <div key={i} className={styles.attr}>
